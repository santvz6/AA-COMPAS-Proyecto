...
\author{
    \textbf{Santiago Álvarez Geanta}\thanks{\texttt{[correo@alu.ua.es]}}
    \quad
    \textbf{Taron Sargsyan}\thanks{\texttt{[correo@alu.ua.es]}}
    \quad
...
Número de \textit{features} (modelado) & 9 (4 numéricas, 5 categóricas)\\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{group_metrics_heatmap}
...
Todos los experimentos se ejecutan con Python~3.10 y semilla fija \texttt{SEED}$=80$ en todos los procesos estocásticos (división \textit{train/test}, optimizador L-BFGS, validación cruzada y ataque adversario). Las versiones principales son: \texttt{scikit-learn}~1.4, \texttt{fairlearn}~0.10, \texttt{shap}~0.45, \texttt{adversarial-robustness-toolbox}~1.17, \texttt{pandas}~2.2, \texttt{numpy}~1.26. El código fuente y los \textit{notebooks} se encuentran en el repositorio público del proyecto (Anexo~B). El siguiente fragmento ilustra la configuración mínima de reproducibilidad:

\begin{lstlisting}[caption={Semilla aleatoria y entorno principal.}]
import numpy as np
SEED = 80
np.random.seed(SEED)
# sklearn.__version__       == '1.4.x'
# fairlearn.__version__     == '0.10.x'
# shap.__version__          == '0.45.x'
# art.__version__           == '1.17.x'
\end{lstlisting}
...
\begin{table}[H]
\centering
\caption{Comparativa de resultados. AUC-ROC y F1: media~$\pm$~desv.\ estándar sobre validación cruzada 5~\textit{folds}. DPD, EOD y FPR diff.\ sobre test (intervalo $[0,1]$, menor es mejor).}
\label{tab:resultados}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Modelo / Configuración} & \textbf{AUC-ROC} & \textbf{F1} & \textbf{DPD} & \textbf{EOD} & \textbf{FPR diff.}\\
\midrule
\textit{Baseline} (Reg. Log.)   & $0{,}XX \pm 0{,}XX$ & $0{,}XX \pm 0{,}XX$ & $0{,}XX$ & $0{,}XX$ & $0{,}XX$ \\
+ Demographic Parity            & $0{,}XX \pm 0{,}XX$ & $0{,}XX \pm 0{,}XX$ & $0{,}XX$ & $0{,}XX$ & $0{,}XX$ \\
+ FPR Parity                    & $0{,}XX \pm 0{,}XX$ & $0{,}XX \pm 0{,}XX$ & $0{,}XX$ & $0{,}XX$ & $0{,}XX$ \\
+ Equalized Odds                & $0{,}XX \pm 0{,}XX$ & $0{,}XX \pm 0{,}XX$ & $0{,}XX$ & $0{,}XX$ & $0{,}XX$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textit{Nota:} los valores marcados como \texttt{XX} deben sustituirse por los resultados obtenidos en la ejecución final del \textit{notebook} \texttt{02\_Modeling\_and\_Fairness.ipynb}; la estructura de la tabla y la interpretación cualitativa son las correctas según los criterios de evaluación descritos en la Sección~\ref{sec:metodologia}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fairness_comparison}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{tradeoff_scatter}
\caption{\textit{Accuracy-fairness trade-off}: cada punto representa un modelo en el plano (\,FPR diff., \textit{accuracy}\,). El punto ideal está arriba a la izquierda.}
\label{fig:tradeoff}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{confusion_matrices}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{robustness_fgm}
