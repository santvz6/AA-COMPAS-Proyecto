...
Tres observaciones se derivan directamente de la Tabla~\ref{tab:resultados}. \emph{Primero}, la diferencia de FPR del \textit{baseline} ($0{,}2055$) confirma cuantitativamente la denuncia de ProPublica: la probabilidad de ser clasificado erróneamente como reincidente es aproximadamente un $20{,}5$ puntos porcentuales mayor para un individuo afroamericano que para uno caucásico. El heatmap de la Figura~\ref{fig:eda} desglosa este resultado: la FPR del grupo afroamericano en el \textit{baseline} es $0{,}388$ frente a $0{,}183$ en el caucásico, es decir, más del doble. \emph{Segundo}, la paridad demográfica logra la mayor reducción absoluta de disparidad ($\text{DPD}=0{,}0312$, $\text{FPR diff.}=0{,}0237$), pero este resultado debe leerse con cautela: el heatmap muestra que lo consigue subiendo la FPR caucásica a $0{,}343$ (desde $0{,}183$) y subiendo simultáneamente la FNR de ambos grupos, lo que equivale a igualar \emph{por empeoramiento general}, no por mejora selectiva. \emph{Tercero}, la \textit{equalized odds} presenta el mayor coste en \textit{accuracy} global ($0{,}6278$, una caída de $3{,}7$ pp respecto al \textit{baseline}), lo cual ilustra empíricamente el teorema de imposibilidad: cuanto más estricta la restricción de equidad simultánea en FPR y FNR, mayor la pérdida de rendimiento agregado.
...
Un resultado especialmente revelador emerge del análisis cruzado de FPR y FNR por grupo (Figura~\ref{fig:eda}). En el \textit{baseline}, el modelo presenta un patrón de error asimétrico y complementario: la FPR del grupo afroamericano ($0{,}388$) duplica la del caucásico ($0{,}183$), pero la FNR sigue el patrón inverso, $0{,}284$ para afroamericanos frente a $0{,}586$ para caucásicos. Es decir, el modelo comete errores \emph{de distinto tipo} según el grupo racial: sobreclasifica como reincidentes a los afroamericanos (exceso de falsos positivos) y subclasifica como reincidentes a los caucásicos (exceso de falsos negativos). Esta asimetría no es aleatoria: es la consecuencia directa de un umbral de decisión único ($\tau$) aplicado a distribuciones de probabilidad predicha que difieren entre grupos por efecto de los \textit{proxies} raciales identificados por SHAP. Al aplicar las intervenciones de equidad, se observa que la paridad demográfica y la \textit{equalized odds} tienden a igualar ambas tasas de error elevando la FPR caucásica (a $0{,}343$ en DP) y elevando la FNR afroamericana (a $0{,}357$ en DP), convergiendo hacia un error distribuido de forma más uniforme, pero globalmente mayor. Este fenómeno ilustra de forma empírica el mecanismo del \texttt{ThresholdOptimizer}: no modifica el modelo subyacente, sino que ajusta umbrales por grupo, \emph{redistribuyendo} los errores en lugar de \emph{reducirlos}.
